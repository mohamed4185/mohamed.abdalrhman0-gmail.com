        ...
        if 'action_wiz' in self.env.context:
            if self.env.context['action_wiz'] == 'depoist':
                ...
                for check in checks:
                    ...
                    analyitc_id=self.env['native.payments.check.create'].search([('id','=',check.check_id)]).nom_pay_id.analyitc_id
                    _logger.info("ANALYTICAL")
                    _logger.info(analyitc_id.name)
                    ...
                    debit_account = []
                    credit_account = []
                    debit_account.append({'account': self.journal_id_deposit.default_debit_account_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id,})
                     
                    if check.notes_rece_id:
                        credit_account.append({'account': check.notes_rece_id.id, 'percentage': 100,'percentage': 100,'analyitc_id' : analyitc_id.id,})
                    else:
                        credit_account.append({'account': check.unit_id.project_id.NotesReceivableAccount.id, 'percentage': 100,'percentage': 100,'analyitc_id' : analyitc_id.id,})
                ...
                for check in checks:
                    ...
                    if check.investor_id:
                        debit_account.append({'account': self.journal_id.default_debit_account_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id,})
                        if check.under_collect_id:
                            credit_account.append({'account': check.investor_id.property_account_receivable_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id,})
                        else:
                            credit_account.append({'account': check.investor_id.property_account_receivable_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id,})

                        if check.state == 'returned':
                            if check.notes_rece_id:
                                credit_account.append({'account': check.notes_rece_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id,})
                            else:
                                credit_account.append(
                                    {'account': check.unit_id.project_id.NotesReceivableAccount.id, 'percentage': 100,'analyitc_id' : analyitc_id.id,})
                ...
                for check in checks:
                    debit_account = []
                    credit_account = []
                    payable_account=self.env['native.payments.check.create'].search([('id','=',check.check_id)]).nom_pay_id.account_id
                    analyitc_id=self.env['native.payments.check.create'].search([('id','=',check.check_id)]).nom_pay_id.analyitc_id
                    payment_journal=self.env['account.journal'].search(['|',('default_credit_account_id','=',payable_account.id),('default_debit_account_id','=',payable_account.id)])
                    ...
                    for approve_ch_line in self.approve_check:
                        if approve_ch_line.check_id == check.id:
                            ...
                            check.open_amount -= approve_ch_line.paid_amt
                    if check.investor_id:
                        debit_account.append({'account': payable_account.id, 'percentage': 100,'analyitc_id' : analyitc_id.id,})
                        #checks_id=self.env['check.cycle.accounts.default'].search([('id','=',check.check_id)])[0]
                        #checks_id_i=self.env['check.cycle.accounts.default'].search([('id','=',check.check_id)])
                        #_logger.info('CHECK ID INFO')
                        #_logger.info(checks_id_i)

                        if not check.deposited_journal:
                            raise exceptions.ValidationError("Cheque must be deposited........")
     
                        if check.under_collect_id:
                            credit_account.append({'account': check.deposited_journal.default_credit_account_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id,})
                        else:
                            credit_account.append({'account': check.deposited_journal.default_credit_account_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id,})
                ...
                for check in checks:
                    ...
                    debit_account = []
                    credit_account = []
                    credit_account.append({'account': check.under_collect_jour.default_debit_account_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id})
                    if check.notes_rece_id:
                        debit_account.append({'account': check.notes_rece_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id})
                    else:
                        debit_account.append({'account': check.unit_id.project_id.NotesReceivableAccount.id, 'percentage': 100,'analyitc_id' : analyitc_id.id})
                    self.sudo().env['create.moves'].create_move_lines(move=move, move_line=move_line,
                                                               debit_account=debit_account,
                                                               credit_account=credit_account,
                                                               src_currency=self.env['res.users'].search([('id', '=', self.env.user.id)]).company_id.currency_id,
                ...
                for check in checks:
                    ...
                    debit_account = []
                    credit_account = []
                    credit_account.append({'account': self.journal_id.default_debit_account_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id})
                    debit_account.append({'account': check.notespayable_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id})
                    self.sudo().env['create.moves'].create_move_lines(move=move, move_line=move_line,
                                                               debit_account=debit_account,
                                                               credit_account=credit_account,
                                                               src_currency=self.env['res.users'].search([('id', '=', self.env.user.id)]).company_id.currency_id,
                    ...
                    credit_account.append({'account': journal_id.account_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id})
                    debit_account.append({'account': check.investor_id.property_account_payable_id.id, 'percentage': 100,'analyitc_id' : analyitc_id.id})
                    self.sudo().env['create.moves'].create_move_lines(move=move, move_line=move_line,
                                                               debit_account=debit_account,
                                                               credit_account=credit_account,
                                                               src_currency=self.env['res.users'].search([('id', '=', self.env.user.id)]).company_id.currency_id,
                ...
                for check in checks:
                    ...
                    if check.investor_id:
                        debit_account = [{'account':journal_id.account_id.id , 'percentage' : 100,'analyitc_id' : analyitc_id.id}]
                        credit_account = [{'account': journal_id.payment_method.default_credit_account_id.id,'percentage' : 100,'analyitc_id' : analyitc_id.id}]
