    ...
    def action_save(self):
        if 'action_wiz' in self.env.context:
            if self.env.context['action_wiz'] == 'depoist':
                ...
                check_line_val['partner_id'] = self.merge_split_checks[i].partner_id
                ...
                for ins in checks:
                    while cur_chk_amt > 0 :
                        ...
                        rel_vals = {}
                        rel_vals['amount_released'] = cur_chk_amt
                        rel_vals_array.append(rel_vals)
                        for acc_ins in rel_vals_array:
                            acc_ins['check_id'] = new_check.id
