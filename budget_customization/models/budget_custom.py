...
class budget_custom(models.Model):
    ...
    def get_total_amount(self):
        ...
        for rec in self.crossovered_budget_line:
            sum+=rec.planned_amount
        if self.amount!=sum:
            raise ValidationError('Error Total Amount is not equal Planned Amount')
    ...
    def get_date_from(self):
        ...
        if self.date_from and self.date_to:
            ...
            if int(date1.strftime('%d'))>1:
                ...
                list.append((0,0,  {'date_from':self.date_from,'date_to':(date1-timedelta(days=1)).strftime('%Y-%m-%d')}))
