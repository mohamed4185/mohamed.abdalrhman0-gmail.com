...
class AccountAccount(models.Model):
    ...
    def get_duplicate(self):
        records=self.env['account.account'].search([('name','=',self.name)])
        if len(records)>1:
...
#     parent_right = fields.Integer('Right Parent', index=1)
    ...
    @api.multi
    def unlink(self):
        account_parent=self.env['account.account'].search([('parent_id','=',self.id)])
        if account_parent:
             raise ValidationError('This account is parent ..........')
        else :
